...
void	ClapTrap::attack( const std::string& target ) {
	if  ( _energyPoints == 0 ) {
		std::cout << "No more Energy Points available : can't attack!\n";
		return ;
	}
	if ( _hitPoints == 0 ) {
		std::cout << _name << " is already dead : can't attack!\n";
		return ;
	}
...
void	ClapTrap::takeDamage( unsigned int amount ) {
	if ( _hitPoints == 0 ) {
		std::cout << _name << " is already dead : can't take any more damage!\n";
		return ;
	}
...
void	ClapTrap::beRepaired( unsigned int amount ) {
	if  ( _energyPoints == 0 ) {
		std::cout << "No more Energy Points available : can't be repaired!\n";
		return ;
	}
	if ( _hitPoints == 0 ) {
		std::cout << _name << " is already dead : can't be repaired!\n";
		return ;
	}
